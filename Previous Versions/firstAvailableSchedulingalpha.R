firstAvailableScheduling <- function(schedule,patient,Day,crossbook=TRUE) {
      ...
      while(booked!=TRUE) {
            if(schedule[[day]][[team]][provider,col]!=0) {
                  schedule[[day]][[team]][provider,col] <- schedule[[day]][[team]][provider,col]-1
                  booked <- TRUE
      ...
      while(booked!=TRUE) {
            ...
            for(i in 1:(nrow(schedule[[day]][[team]])-1)) {
                  ...
                  if(schedule[[day]][[team]][providerTeam[i],col] != 0) {
                        schedule[[day]][[team]][providerTeam,col] <- schedule[[day]][[team]][providerTeam,col]-1
                        booked <- TRUE
                  } else if(type == "acute" && schedule[[day]][[team]][providerTeam[i],2]!=0) {
                        schedule[[day]][[team]][providerTeam,2] <- schedule[[day]][[team]][providerTeam,2]-1
                        booked <- TRUE
                  }
            }
            ...
            for(j in 1:(length(schedule[[day]])-1)) {
                  ...
                  for(i in 1:(nrow(schedule[[day]][[team]])-1)) {
                        ...
                        if(schedule[[day]][[Team[j]]][providerTeam[i],col]!=0) {
                              schedule[[day]][[Team[j]]][providerTeam,col] <- schedule[[day]][[Team[j]]][providerTeam,col]-1
                              booked <- TRUE
                        } else if(type == "acute" && schedule[[day]][[Team[j]]][providerTeam[i],2]!=0) {
                              schedule[[day]][[Team[j]]][providerTeam,2] <- schedule[[day]][[Team[j]]][providerTeam,2]-1
                              booked <- TRUE
                        }
                  }
            }
...
returnFunc <- list(schedule, POP, provPOP, teamPOP)  
returnFunc
#function returns a list with:
#1. schedule
#2. POP (if access was met)
